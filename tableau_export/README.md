...
| `events_elections.csv`, `events_budgets.csv` | Passthrough of the raw event date/label tables, kept as separate files for reference. |
| `events_combined.csv` | Elections + budgets in one table with an `event_type` column (Election/Budget). Use this one for the "rule out politics" chart: a single confidence-timeline overlay with both event types colour-coded and a filter/legend toggle to show one, both, or neither, instead of two near-identical charts. |
